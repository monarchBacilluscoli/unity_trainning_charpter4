...
public class OrbitCameraWithGravityChange : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        ) * gravityAlignment; //? 为什么要乘上当前摄像机的旋转？为什么不直接用。哦，因为当前是基旋转，前面求的是delta，当然要+一下了
        // 更新当前跟随目标
        UpdateFocusPoint();
        // 只有在输入/自动调整有效的情况下
        ...
        // 得到相机注视旋转
        Quaternion lookRotation = gravityAlignment * m_orbitRotation;
    ...
    bool AutomaticRotation()
    {
        ...
        // 更新上次位移
        Vector3 alignedDelta = Quaternion.Inverse(gravityAlignment) * (m_focusPoint - m_previousFocusPoint);
