...
public class MovingSphere2 : MonoBehaviour
{
    ...
    void AdjustVelocity()
    {
        ...
        // 5. 将当前速度沿平面的方向进行分配，而非沿水平x、z进行分配并更新速度
    ...
    bool SnapToGround()
    {
        ...
        // 6. 如果需要进行吸附，那么将地面技术设为1，并设置地面法线为当前hit的normal
        ...
        // 8. 如果向下坡方向移动，则求得速度沿斜面的方向的单位向量，并和速度绝对值相乘得到速度方向
        if (dot > 0f)
        {
